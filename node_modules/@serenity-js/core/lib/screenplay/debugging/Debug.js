    ...
    async performAs(actor) {
        ...
        for (const value of this.values) {
            ...
            actor.collect(model_1.LogEntry.fromJSON({
                data: JSON.stringify({
                    value: (0, stringified_1.stringified)(result.value),
                    error: result.error
                        ? errors_1.ErrorSerialiser.serialise(result.error)
                        : result.error
                }),
            }), new model_1.Name(result.description));
            results.push(result);
        }
        this.debuggerFunction(results, ...results.map(result => result.value));
    }
}
exports.Debug = Debug;
